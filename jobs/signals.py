...
def generate_price_adjustment(instance):
    ...
    if (
        instance.set_price_adjustment
        and isinstance(instance.price_adjustment, type(None)) == False
    ):
        # set price
...
def job_invoice_issue(instance):
    ...
    if instance.job_invoice_sent or isinstance(instance.pk, type(None)):
        return

    # ...
    clientNameAndSurname = "{0} {1}".format(
        instance.customer.f_name, instance.customer.s_name
    )

    # make sure instance.job_date and instance.job_time exist
    ...
    templateData = {
        ...
        "clientPhoneNumber": instance.customer.m_number,
        "clientEmail": instance.customer.email,
        "invoiceDate": instance.created_at.strftime("%d/%m/%Y"),
        "jobDateTime": f"{job_date} {job_time}",
        "helpers": instance.helpers,
        "floors": instance.floors,
        "distance": instance.distance,
        "vehicleType": instance.vehicle_size,
        "routes": instance.routes.all(),
        # "jobLink": f"https://xlines.co.za/jobs/job/{instance.pk}/update",
    }

    # ... this is where I send the email
    htmlContent = render_to_string("invoice.html", templateData)
...
def jobCancellation(instance):
    ...
    # .. check job cancelled
    if instance.job_canceled and instance.job_cancellation_feedback_sent == False:
        # .. here we send email to customer and admin
        # .. compute customer name cName = Customer Name
        cName = "{0} {1}".format(instance.customer.f_name, instance.customer.s_name)
...
def generate_return_customer_discount(new_quote, user_account):
    ...
    if isinstance(user_account, get_user_model()):
        ...
        # .. continue
        customerJobs = Job.objects.filter(customer=user_account).order_by("-created_at")

        # .. reffer
        if len(customerJobs) > 1:
            return new_quote * (RETURN_CUSTOMER_DISCOUNT / 100.0)
...
def before_saved(sender, instance, *args, **kwargs):
    ...
    if isinstance(instance, Job):
        ...
        rCustomerDiscount = generate_return_customer_discount(
            (customerQuote - mid_discount), instance.customer
        )

        # generate extra given discount
        extra_discount_amount = generate_extra_given_discount(
            instance, (customerQuote - mid_discount - rCustomerDiscount)
        )
